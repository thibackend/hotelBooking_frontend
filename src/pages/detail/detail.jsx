...
function Detail() {
  ...
  const fetchRoom = async () => {
    await axios
      .get(`http://localhost:8000/api/getOne-room-and-images/${id}`)
      .then((res) => {
        if (res.data) {
            ...
            <div className="col-sm-12">
              ...
              >
                {images ? (
                  images.map((e, index) => (
                    <img
                      key={index}
                      src={`http://127.0.0.1:8000/uploads/images/${e}`}
                      alt="Anh tượng trung"
                    />
                  ))
                ) : (
              ...
              <div className="h-25 p-3 d-text">
                <div className="d-sm-flex flex-row justify-content-start">
                  ...
                  <div className="black-border rounded  border-warning  hover-shadow">
                    Perfect
                  </div>
                  <div className="black-border rounded  border-primary">
                    Top Value
                  </div>
                  <div className="black-border rounded  border-primary">
                    Vip room
          ...
          >
            {roomRelevant && roomRelevant.length === 1 ? (
              ...
              roomRelevant.map((e, index) => (
                <div key={index}>
                  <a href={`/detail/${e.id}`} variant="success">
                    <Card style={{ width: "20rem" }}>
                      <Card.Img
                        variant="top"
                        src={
